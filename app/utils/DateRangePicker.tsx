...
export default function DateRangePicker({value, onChange}: Props) {
  ...
  const displayRange = Platform.OS === 'android' ? value : tempRange;
  ...
  const onDateChange = (event: any, selectedDate?: Date) => {
    ...
    if (selecting === 'start') {
      setTempRange({...tempRange, startDate: selectedDate});
      setSelecting('end');
      if (Platform.OS === 'android') setAndroidPicker(true);
    } else {
      setTempRange({...tempRange, endDate: selectedDate});
      setShow(false);
      onChange({startDate: tempRange.startDate, endDate: selectedDate});
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.rangeBox} onPress={openPicker}>
        <Text style={styles.rangeText}>
          {`${displayRange.startDate.toLocaleDateString()} - ${displayRange.endDate.toLocaleDateString()}`}
        </Text>
        <View style={{width: 8}} />
        <CalendarIcon />
      </TouchableOpacity>
          ...
          <View style={styles.modalBg}>
            <View style={styles.pickerBox}>
              ...
              <DateTimePicker
                ...
                minimumDate={
                  selecting === 'end' ? tempRange.startDate : undefined
                }
              />
              <TouchableOpacity
                style={styles.cancelBtn}
                onPress={() => setShow(false)}>
                <Text style={{color: '#4299E1'}}>Cancel</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
      ...
      {Platform.OS === 'android' && androidPicker && (
        <DateTimePicker
          value={
            selecting === 'start' ? tempRange.startDate : tempRange.endDate
          }
          mode="date"
          display="default"
          onChange={onDateChange}
          minimumDate={selecting === 'end' ? tempRange.startDate : undefined}
        />
      )}
    </View>
  );
